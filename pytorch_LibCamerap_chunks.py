...
def object_detection(image, model, classes, side):
    ...
    if confidence > 25:  # Threshold for detection
        print(f"side: {side} Detected: {detected_object} with confidence {confidence:.2f}% at frame {global_frame_count}")
...
cv2.namedWindow("left stream", cv2.WINDOW_AUTOSIZE)
cv2.namedWindow("right stream", cv2.WINDOW_AUTOSIZE)

half_image_width = 112
...
with torch.no_grad():
    while True:
        try:
            ...
            right_section = image[:,:half_image_width]
            left_section = image[:,half_image_width:]

            # Convert RGB to BGR for OpenCV display
            bgr_image_left = cv2.cvtColor(left_section, cv2.COLOR_RGB2BGR)
            bgr_image_right = cv2.cvtColor(right_section, cv2.COLOR_RGB2BGR)
            cv2.imshow("left stream", bgr_image_left)
            cv2.imshow("right stream", bgr_image_right)
            
            # Process key events - Exit on 'q' press
            if cv2.waitKey(1) & 0xFF == ord('q'):
                break
            
            object_detection(left_section, model, classes, "left")
            object_detection(right_section, model, classes, "right")
